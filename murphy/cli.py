...
def main() -> int:
	...
	parser.add_argument("--no-highlights", action="store_true", help="Disable bounding boxes on interactive elements in the browser")
...
async def _async_main(args: argparse.Namespace) -> None:
	...
	try:
		...
		browser_session = BrowserSession(browser_profile=BrowserProfile(keep_alive=True, dom_highlight_elements=not args.no_highlights))
		...
		if args.plan:
			...
			if args.features:
				...
				print(f"  Review and edit the file, then press Enter to continue.")
				print(f"  (Add, remove, or modify features as needed.)\n")
			...
			print(f"  Review and edit the file, then press Enter to continue.")
			print(f"  (Add, remove, or modify test scenarios as needed.)\n")
		...
		# ── Phase 3: Execute ──
		if not args.ui:
			results = await _execute_with_session(args.url, test_plan, llm, browser_session)
			if analysis:
		...
		async def _execute_fn(plan: TestPlan, state: "ServerState") -> list[TestResult]:
			return await _execute_with_session(
				args.url, plan, llm, _browser_session, progress_state=state
			)

		state = ServerState(
			url=args.url,
			analysis=analysis or WebsiteAnalysis(
...
async def _detect_auth_required(
	browser_session: "BrowserSession", llm: "ChatOpenAI", url: str
) -> bool:
...
async def _llm_classify_page(
	llm: "ChatOpenAI", url: str, title: str, body: str, *, mode: str = "auth_detect"
) -> bool:
...
async def _analyze_with_session(
	url: str, llm: "ChatOpenAI", browser_session: "BrowserSession", goal: str | None = None,
) -> "WebsiteAnalysis":
...
async def _execute_with_session(
	...
	"""Phase 3 execution reusing an authenticated browser session."""
	from browser_use import Agent

	from murphy.evaluate import classify_failure
	from murphy.judge import murphy_judge
	from murphy.models import TestResult
...
def _read_features_markdown(path: Path) -> "WebsiteAnalysis":
	...
	for line in lines:
		...
		if in_pages_table and line.startswith("|") and not line.startswith("|---") and not line.startswith("| Page"):
			...
			if len(cells) >= 3:
				...
				valid_types = ('homepage', 'landing', 'product', 'listing', 'detail',
					'form', 'content', 'dashboard', 'auth', 'error', 'other')
				if page_type not in valid_types:
					page_type = 'other'
				key_pages.append(PageInfo(
					url=url, title=title, purpose=purpose,
					page_type=page_type, interactive_elements=[],  # type: ignore[arg-type]
				))
	...
	while i < len(lines):
		...
		if feat_match:
			...
			# Valid categories
			valid_cats = ('navigation', 'search', 'forms', 'content_display',
				'filtering_sorting', 'media', 'authentication', 'ecommerce', 'social', 'other')
