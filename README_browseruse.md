...
**Run Murphy (AI-driven website evaluation):**
```bash
...
# Public site, skip auth detection
murphy --url https://example.com --no-auth

# Resume from a previously generated features file or test plan
...
# Other options
murphy --url https://example.com --max-tests 5       # limit number of test scenarios (default: 8)
murphy --url https://example.com --model gpt-4o       # choose model (default: gpt-4o)
murphy --url https://example.com --output-dir ./out    # custom output directory
murphy --url https://example.com --ui                  # launch interactive web UI
```

**Run tests:**
```bash
uv run pytest -vxs tests/ci                    # full CI suite
uv run pytest -vxs tests/ci/interactions/      # just interaction tests
uv run pytest -vxs tests/ci/test_specific.py   # single test file
```
